...
public class ClassCoverageScorer implements IVariableOntologyScorer {
	...
	public void score(IOntology o, Multiset<String> classIds) {
		...
		if (o != null && classIds != null) {
			...
			// Multiset
			Multiset<String> classes = HashMultiset.create(o.getClassIds()
					.size());
			classes.addAll(o.getClassIds());

			// the classes of the input ontology that cover the input terms
			Multiset<String> coveringClasses = Multisets.filter(classes,
					Predicates.in(classIds));
			...
			// the given ontology
			Multiset<String> coveredTerms = Multisets.filter(classIds,
					Predicates.in(coveringClasses));
			...
			o.setScore(
					ScoreType.TERM_COVERAGE,
					Double.valueOf((double) numOfCoveredTerms
							/ (double) numOfTerms));
		...
		if (s != null && classIds != null) {
			...
			for (IOntology o : s.getOntologies()) {
				...
				log.debug("coveringClasses for {}: {}", o.getId(),
						null == coveringClasses ? 0
								: coveringClasses.size());
				if (overallCoveringClasses == null) {
					overallCoveringClasses = coveringClasses;
				} else {
					overallCoveringClasses = Multisets.sum(
							overallCoveringClasses, coveringClasses);
					log.debug("final overallCoveringClasses for {}: {}",
							o.getId(), overallCoveringClasses.size());
				}
			...
			// cache the set of covering classes
			s.addCacheInformation(InfoType.COVERING_CLASSES,
					overallCoveringClasses);

			// the number of input terms covered by the classes defined within
			// the given ontology
			Multiset<String> coveredTerms = Multisets.filter(classIds,
					Predicates.in(overallCoveringClasses));
			int numOfCoveredTerms = coveredTerms.size();
			...
			s.setScore(
					ScoreType.TERM_COVERAGE,
					Double.valueOf((double) numOfCoveredTerms
							/ (double) classIds.size()));
			log.debug("Covering score: {}", s.getScore(ScoreType.TERM_COVERAGE));
		} else {
			log.warn(
					"Could not compute a class coverage score for an ontology set. OntologySet: {}, classIds: {}",
					s, classIds);
		}
	...
	public Double getScoreAdded(IOntologySet s, IOntology o,
		...
		if (s != null && classIds != null && o != null) {
			...
			return Double.valueOf((double) numOfCoveredTerms
					/ (double) classIds.size());
	...
	public Double getScoreRemoved(IOntologySet s, IOntology o,
		...
		if (s != null && classIds != null && o != null) {
			...
			Multiset<String> overallCoveringClasses = Multisets.difference(
					coveringClassesOfS, coveringClassesOfO);
